...
class AppUserMenu extends StatelessWidget {
  ...
  factory AppUserMenu.popup({
    ...
    user: user,
    actions: {
      'profile': UserMenuAction(label: 'Perfil', icon: Icons.person_outline, onTap: onProfileTap),
  ...
  Widget _buildInlineMenu(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        ...actions.entries.map((entry) => _buildInlineMenuItem(context, entry.key, entry.value)),
      ],
    );
  }
  ...
  Widget _buildCompactMenu(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      itemBuilder: (BuildContext context) {
        return actions.entries.map((entry) => _buildPopupMenuItem(entry.key, entry.value)).toList();
      },
      child: AppAvatar.small(imageUrl: user?.avatarUrl, initials: _getUserInitials()),
    );
  }
    ...
    return PopupMenuItem<String>(
      ...
      child: Row(
        children: [
          ...
          Text(
            action.label,
            style: TextStyle(color: action.isDangerous ? AppTheme.errorColor : null),
          ),
        ],
      ),
    );
  ...
  Widget _buildInlineMenuItem(BuildContext context, String key, UserMenuAction action) {
    return ListTile(
      leading: Icon(
        action.icon,
        size: LayoutConstants.iconMedium,
        color: action.isDangerous ? AppTheme.errorColor : (textColor ?? AppTheme.onSurface),
      ),
      title: Text(
        action.label,
        style: TextStyle(
      ...
      onTap: action.onTap,
      contentPadding: EdgeInsets.symmetric(horizontal: LayoutConstants.paddingXs, vertical: 0),
    );
  }

  Widget _buildMenuTrigger(BuildContext context) {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: LayoutConstants.marginXs),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          AppAvatar(radius: avatarRadius, imageUrl: user?.avatarUrl, initials: _getUserInitials()),
...
enum UserMenuVariant { popup, inline, compact }
