...
class NavigationSidebar extends ConsumerWidget {
    ...
    try {
      ...
      for (final rootItem in rootItems) {
        ...
        if (subItems.isEmpty) {
          ...
          menuWidgets.add(_buildExpandableMenuItem(context, rootItem, subItems));
        }
      }
  ...
  Widget _buildExpandableMenuItem(BuildContext context, dynamic rootItem, List<dynamic> subItems) {
    final isAnySubItemActive = subItems.any(
      (subItem) => _isRouteActive(subItem.route, currentRoute),
    );
      ...
      child: ExpansionTile(
        ...
        title: Text(
          ...
          style: TextStyle(
            ...
            fontSize: LayoutConstants.fontSizeMedium,
            fontWeight: (isRootActive || isAnySubItemActive) ? FontWeight.w600 : FontWeight.w400,
          ),
        ),
