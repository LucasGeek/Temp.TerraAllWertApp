    ...
    return Row(
      ...
      children: [
        ...
        if (showText) ...[
          ...
          Text(
            'Terra Allwert',
            style: _getTextStyle(context, effectiveSize),
          ),
        ],
      ],
    );
...
class _TerraLogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (variant != LogoVariant.minimal) {
      ...
      _drawHexagonFace(canvas, paint, center + Offset(radius * 0.3, -radius * 0.2), radius * 0.8, 0);
    }

    // Face superior (mais escura) - apenas para versões completas
    if (variant == LogoVariant.standard) {
      paint.color = tertiaryColor;
      _drawHexagonFace(canvas, paint, center + Offset(radius * 0.15, -radius * 0.4), radius * 0.9, 0);
    }
