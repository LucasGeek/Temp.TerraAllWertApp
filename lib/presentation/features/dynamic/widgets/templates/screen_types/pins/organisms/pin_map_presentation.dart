...
import '../../../../../../../layout/widgets/atoms/offline_image.dart';
import '../../../../../../../providers/permissions_provider.dart';
import '../../../../../../../utils/notification/snackbar_notification.dart';
import '../../../../../providers/pin_map_provider.dart';
...
class _PinMapPresentationState extends ConsumerState<PinMapPresentation> {
  ...
  // Variável de storage (mock para demonstração)
  final dynamic _pinCacheAdapter = null;
  ...
  /// Carrega os dados do mapa usando o provider
  void _loadMapData() {
    // Usar o provider em vez do storage direto
    ref.read(pinMapNotifierProvider(widget.route).notifier).loadMapData();
  }

  /// Salva os dados do mapa usando o provider
  Future<void> _saveMapData() async {
    // Usar o provider para salvar dados
    await ref.read(pinMapNotifierProvider(widget.route).notifier).saveMapData();
  ...
  Widget build(BuildContext context) {
    ...
    if (pinMapState.hasError || pinMapState.mapData == null) {
      AppLogger.debug(
        'BUILD pinMapState: isLoading=${pinMapState.isLoading}, hasError=${pinMapState.hasError}, mapData=${pinMapState.mapData != null ? "não-null" : "null"}',
        tag: 'PinMap',
      );
    }
    ...
    // Só mostra erro se realmente houve um erro de carregamento (não apenas dados null)
    if (_hasError) {
      AppLogger.warning('Mostrando error state por hasError=true', tag: 'PinMap');
      return _buildErrorState();
    }

    // Se mapData é null mas não há erro, criar dados temporários para evitar null reference
    _mapData ??= InteractiveMapData(
      id: 'temp_${widget.route}',
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        Text(
                          '🎯 Como criar seu mapa interativo:',
                          style: TextStyle(
                            color: AppTheme.primaryColor,
                            fontSize: LayoutConstants.fontSizeLarge,
                            fontWeight: FontWeight.w600,
                        ...
                        _buildInstructionStep(
                          '1',
                          'Clique em "Começar a Editar"',
                          'Ative o modo de edição para personalizar o mapa',
                        ),
                        _buildInstructionStep(
                          '2',
                          'Adicione imagem de fundo',
                          'Faça upload de uma planta, mapa ou foto como base',
                        ),
                        _buildInstructionStep(
                          '3',
                          'Adicione pins interativos',
                          'Toque na imagem para marcar pontos importantes',
                        ),
                        _buildInstructionStep(
                          '4',
                          'Configure cada pin',
                          'Adicione títulos, descrições e imagens aos pins',
                        ),
                      ],
                    ),
                  ),
